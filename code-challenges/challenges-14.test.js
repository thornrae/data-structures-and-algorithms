...
const toTitleCase = (arr) => {
  return arr.map(word => {
    ...
    //dont really understand how this works, found it on stack overflow
...
let biggerThanLuke = (arr) => {
  // do you convert mass that is a string to a number? how? parsInt
  let larger = [];
  arr.filter(person => {
    let massNum = parseInt(person.mass);
    // console.log(massNum, person.name);
    if(massNum > 77){
      larger.push(person.name);
    }
  });
  // let largerString = larger.toString();
  // console.log('largerstring....', largerString);
  let stringit = larger.join(' - ');
  return stringit;
};

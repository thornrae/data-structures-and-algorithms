...
const createServer = () => {
  ...
  app.get('/aboutme', (request, response) => {
    response.status(200).send('my name is taylor');
  });

  app.get('/favoritefoods', (request, response) => {
    response.status(200).send('my favorite food is poke');
  });
...
const count = (target, input) => {
  var counter = 0;
  input.filter( firstloop => {
    firstloop.filter( secondloop => {
      if(secondloop === target){
        counter++;
      }
    });
  });

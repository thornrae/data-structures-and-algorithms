...
function getCurrentEvents(request, response){
  ...
  //this line was causing check mark 3 to not work, removed because not necessary because reference currentEvents object in mapCurrentEvents() I THINK??!

  // const newsArr = [];
  // currentEvents.news.forEach( info => {
  let mapped = mapCurrentEvents();
...
const mapCurrentEvents = () => {
  return currentEvents.news.map(newsData => {
    return new Event(newsData);
  });
};
...
//found this solution thru research, could not explain how it works. 
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
...
const reversedString = (str) => {
  ...
  return splitStr.reduce( (newString, currentLetter) => {
...
const countNumberOfChildren = (arr) => {
  ...
  //select children 
  //push all children to single array
